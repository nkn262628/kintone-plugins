(function () {
  ...
  function renderReturnSelector(currentRefNo) {
    ...
    kintone.api(kintone.api.url('/k/v1/records', true), 'GET', { app: SELF_APP_ID, query: query }).then(resp => {
      ...
      sel.addEventListener('focus', () => { sel.style.borderColor = '#7d8bae'; });
  ...
  function renderHistoryPanel(refNo) {
    ...
    kintone.api(kintone.api.url('/k/v1/records', true), 'GET', { app: SELF_APP_ID, query: query }).then(resp => {
      ...
      panel.style.cssText = [
        'margin:0', 'background:#fff', 'border:1px solid #e5e8ea', 'border-left:4px solid #45496a',
        'border-radius:10px', 'box-shadow:0 2px 8px rgba(0,0,0,.06)', 'font-size:13px', 'overflow:hidden',
        'width:100%', 'box-sizing:border-box'
      ].join(';');

      const header = document.createElement('div');
      header.style.cssText = 'display:flex;align-items:center;gap:10px;padding:16px 24px;background:#f7f6f4;border-bottom:1px solid #eef1f2;';
      ...
      records.forEach(r => {
        ...
        const dotColor = r[F_STATUS].value === ST_DONE ? '#4c7a63' : '#d1d5db';
        const tagBg = isLend ? '#eef1f7' : '#fbedeb';
        const tagColor = isLend ? '#45496a' : '#a8483c';
      ...
      return validateReturnPromise.then(returnCheck => {
        ...
        rows.forEach(row => {
          ...
          if (sfExists) {
            ...
            if (qty > realAvail) {
              row.value[F_QTY].error = '剩餘可用庫存不足，目前最多可調整為 ' + realAvail;
              hasStockError = true;
            }
          }
        });
